...
def get_metrics():
    ...
    try:
        ...
        # Map range types to number of days
        range_map = {
            '7d': 7,
            '30d': 30,
            '90d': 90,
        ...
        days = range_map.get(range_type, 7)
        ...
        with get_db_context() as db:
            ...
            for row in daily_stats_query:
                ...
                calls_by_day.append({"date": date_str, "calls": row.total_calls})
                # Mock revenue: $10 per completed call
                revenue_by_day.append({"date": date_str, "revenue": (row.completed_calls or 0) * 10})

            # Mock total revenue for now as it's not in the model
